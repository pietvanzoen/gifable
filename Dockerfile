...
COPY --from=ts-compiler /usr/app/_scripts ./_scripts
RUN yarn --production
...
HEALTHCHECK --interval=30s --timeout=30s --start-period=10s --retries=3 CMD wget -q -O /dev/null http://localhost:3000/health || exit 1
